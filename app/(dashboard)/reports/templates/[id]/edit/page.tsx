...
import type { ReportTemplate, SlotDefinition, TemplateType, SlotType, SlotDataSource } from '@/lib/types'
...
export default function EditTemplatePage() {
  ...
  const [form,      setForm]      = useState<TemplateForm>({
    name: '', template_type: 'narrative', description: '', agent_instructions: '',
    slots: [], design_tokens: {}, scaffold_html: '', scaffold_css: '', scaffold_js: '',
  })
  ...
  const load = useCallback(async () => {
    try {
      ...
      const t = json.data as ReportTemplate
      setForm({
        name:               t.name,
        ...
        scaffold_html:      t.scaffold_html ?? '',
        scaffold_css:       t.scaffold_css ?? '',
        scaffold_js:        t.scaffold_js ?? '',
      })
    } catch (e) {
      setError(e instanceof Error ? e.message : 'Error loading template')
    } finally {
      setLoading(false)
    }
  }, [templateId])
  ...
  async function handleSave() {
    ...
    try {
      ...
      router.push(`/reports/templates/${templateId}`)
    } catch (e) {
    ...
    <div className="space-y-6 max-w-4xl">
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-center gap-3">
          <Link href={`/reports/templates/${templateId}`} className="text-muted-foreground hover:text-foreground">
            <ChevronLeft className="h-5 w-5" />
          </Link>
          <div>
            <h1 className="text-xl font-bold text-foreground">Edit Template</h1>
        ...
        <div className="space-y-4">
          <Card>
            <CardContent className="p-0">
              {form.slots.length === 0 ? (
                <div className="text-center py-10 text-muted-foreground text-sm">No slots defined.</div>
              ) : (
                  ...
                  <tbody>
                    {form.slots.map((slot, i) => (
                      <tr key={i} className="border-b border-border/40 last:border-0 hover:bg-muted/30">
                        ...
                        <td className="py-2.5 px-4"><Badge variant="outline" className="text-xs">{slot.type}</Badge></td>
      ...
      {tab === 'tokens' && (
        <div className="space-y-4">
          <Card>
            <CardContent className="p-0">
              {Object.keys(form.design_tokens).length === 0 ? (
                <div className="text-center py-10 text-muted-foreground text-sm">No design tokens defined.</div>
              ) : (
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b text-muted-foreground text-xs">
          ...
          <div className="flex items-center gap-2">
            <input value={newTokenKey} onChange={e => setNewTokenKey(e.target.value)} placeholder="token-name"
            ...
            <input value={newTokenValue} onChange={e => setNewTokenValue(e.target.value)} placeholder="#0ea5e9"
              className="flex-1 text-sm font-mono rounded-md border bg-background px-3 py-2 text-foreground placeholder:text-muted-foreground" />
            <button onClick={addToken} disabled={!newTokenKey.trim() || !newTokenValue.trim()}
      ...
      {tab === 'scaffold' && (
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-3">
            <ScaffoldEditor label="HTML"        value={form.scaffold_html} onChange={v => update('scaffold_html', v)} placeholder="<div>{{slot_name}}</div>" />
            <ScaffoldEditor label="CSS"         value={form.scaffold_css}  onChange={v => update('scaffold_css', v)}  placeholder="body { background: {{bg-primary}}; }" />
            <ScaffoldEditor label="JS (optional)" value={form.scaffold_js} onChange={v => update('scaffold_js', v)}   placeholder="// Optional interactive behaviour" />
          </div>

          {previewHtml && (
            <div className="space-y-1.5">
              <p className="text-xs font-medium text-muted-foreground">Preview</p>
              <iframe ref={iframeRef} srcDoc={previewHtml} sandbox="allow-scripts allow-same-origin"
                className="w-full h-64 rounded-lg border bg-white" title="Template preview" />
            </div>
          )}
        </div>
      )}
          ...
          <div className="bg-background rounded-xl border shadow-xl w-full max-w-md space-y-4 p-6">
            ...
            <div className="space-y-3">
              <Field label="Name (snake_case) *">
                <input value={slotModal.slot.name}
              ...
              <Field label="Label *">
                <input value={slotModal.slot.label}
              ...
              <div className="grid grid-cols-2 gap-3">
                <Field label="Type">
                  <select value={slotModal.slot.type}
                ...
                <Field label="Data Source">
                  <select value={slotModal.slot.data_source}
              ...
              <Field label="Description">
                <textarea value={slotModal.slot.description}
                  onChange={e => setSlotModal(m => ({ ...m, slot: { ...m.slot, description: e.target.value } }))}
                  rows={2} placeholder="What does this slot contain?"
                  className="w-full text-sm rounded-md border bg-background px-3 py-2 text-foreground placeholder:text-muted-foreground resize-none" />
              </Field>
              <label className="flex items-center gap-2 text-sm text-foreground cursor-pointer">
                <input type="checkbox" checked={slotModal.slot.required}
            ...
            <div className="flex justify-end gap-2 pt-2">
              <button onClick={() => setSlotModal(m => ({ ...m, open: false }))}
                className="px-4 py-2 rounded-md border text-sm text-muted-foreground hover:text-foreground">Cancel</button>
              <button onClick={saveSlot} disabled={!slotModal.slot.name.trim() || !slotModal.slot.label.trim()}
