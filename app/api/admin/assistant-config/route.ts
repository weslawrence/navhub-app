...
/**
 * GET   /api/admin/assistant-config — fetch the platform-level row (group_id IS NULL)
 * PATCH /api/admin/assistant-config — upsert the platform-level row
 *
 * super_admin only. For per-group config use /api/admin/assistant-config/groups/[id].
 */
...
export async function GET() {
  ...
  const { data } = await admin
    ...
    .select('id, persona_name, persona_tone, persona_instructions, scope_text, knowledge_text, restrictions, is_active, updated_at')
    .is('group_id', null)
    .maybeSingle()
