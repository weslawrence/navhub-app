...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const xeroData = await fetchXeroReport(access_token, xero_tenant_id, 'balance_sheet', period)
    const financialData = normaliseFinancialData(xeroData, 'balance_sheet', period)
    ...
    const { data: snapshot, error: snapshotError } = await admin
      ...
      .upsert(
        {
          period,
          report_type: 'balance_sheet' as const,
    ...
    await admin.from('sync_logs').insert({
      ...
      message: `Balance Sheet synced for ${period}`,
    ...
    await admin.from('sync_logs').insert({
      ...
      message: `Balance Sheet sync failed: ${message}`,
      company_id: connection.company_id, division_id: connection.division_id,
    })
    console.error('Balance Sheet sync error:', err)
